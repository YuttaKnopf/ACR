...
def test_saturation_disruption(
    ...
    saturation_disruption("db", image_path, 5)
    mock_imread.assert_called_once_with("test/mock_img.png")
    mock_saturation_check.assert_called_once_with("image")
...
def test_saturation_check(mock_saturated_pixels_use_grid, mock_percent):
    image = mock_image()
...
@patch("modules.saturation.saturation_algorithm.GRID_SIZE", 50)
...
def test_saturation_check_use_grid(mock_saturated_square):
    image = mock_image()
    sum_saturated_pixels, saturated_squares = saturation_check_use_grid(image)
...
@patch("modules.saturation.saturation_algorithm.SATURATION_SQUARE_PERCENT", 25)
...
def test_saturated_square(mock_percent, mock_calculate_saturation):
    image = mock_image()
    saturated_squares = []
    saturated_pixels = saturated_square(image, 1, 1, saturated_squares)
    assert saturated_pixels == 300
    assert saturated_squares == [[(1, 1), (51, 51)]]


@patch(
    "modules.saturation.saturation_algorithm.SATURATION_THRESHOLD_VALUE",
    [255, 255, 255],
)
def test_all_pixels_white():
    square = np.full((10, 10, 3), [255, 255, 255], dtype=np.uint8)
    assert calculate_saturation(square) == 100
...
def test_no_pixels_white():
    ...
    assert calculate_saturation(square) == 0
...
def test_some_pixels_white():
    ...
    assert calculate_saturation(square) == 2


@patch("modules.saturation.saturation_algorithm.SATURATION_DISRUPTION_PERCENT", 25)
def test_is_saturation_image():
    image = mock_image()
    saturation_image = is_saturation_image(image, 5000)
